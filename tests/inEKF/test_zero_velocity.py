...
def test_noise_reduces_to_rotated_sigma_c():
    ...
    N_v = zv.velocity_noise(state, sigma_c, J, jnp.zeros(N_J), h, jnp.zeros((3, 3)))
...
def test_noise_is_spd_and_grows_with_every_term():
    ...
    small = zv.velocity_noise(state, sigma_c, J, jnp.zeros(N_J), h, jnp.zeros((3, 3)))
    big = zv.velocity_noise(state, sigma_c, J, jnp.full(N_J, 1e-2), h,
                            jnp.eye(3) * 1e-4)
...
def test_fused_block_equals_the_stacked_update():
    ...
    sigma_c = jnp.asarray(L @ np.swapaxes(L, -1, -2))
    y_v = zv.velocity_measurement(omega, h, J, q_dot)
    N_v = zv.velocity_noise(state, sigma_c, J, jnp.full(N_J, 1e-4), h, jnp.eye(3) * 1e-5)
