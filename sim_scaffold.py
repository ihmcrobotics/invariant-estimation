...
def build_sim_model(weld_base=False):
    ...
    print(f"model: nq={m.nq} nv={m.nv} nu={m.nu} actuators, base is {'welded' if weld_base else 'floating'}")
    return m, home, ctrl0

# Servo test
m, home, ctrl0 = build_sim_model(weld_base=True)
d = mujoco.MjData(m)
...
m, home, ctrl0 = build_sim_model(weld_base=False)
d0 = mujoco.MjData(m)
for name, h in home.items():
...
print(f"per env final base-z: {np.round(np.asarray(zsN[:, -1]),3)}")
