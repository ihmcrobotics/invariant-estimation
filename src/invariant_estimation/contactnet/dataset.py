...
Three passes, separated by what they cost and what they depend on:

1. **`build_channel_cache`** — the only pass that needs MJX. Writes ``(T, N_c, F)``
   channels plus ``(T, N_c, 3)`` FK vectors to ``data/cache/`` (~24 MB per rollout
   against the 130 MB rollout itself).
...
axis; at ``T = 62 000`` that reached 38 GB of RSS. Pass 1 therefore goes through
`collect.contact_channels_chunked` (and `chunked_fk`), which is why the cache
exists at all: it converts a pass that cannot be run casually into one `np.load`.
...
Run 1 froze it at the stance constant; see `ContactNetConfig.freeze_contact_chol`
for the full refutation and the measurement that retired it.
...
permutation of the rollouts every batch and cycles it. Starts within a rollout
are uniform random, never tiled — tiled starts at stride ``L`` would make
consecutive batches replay one trajectory in order.

**(e) Ticks inside a segment are never shuffled.** The segment is a trajectory;
`make_segment` only ever slices contiguously.
...
`state0` is therefore the *chain seed*, used at chain start and re-seed only.
Run 1 re-seeded every segment from ground truth — the force-teacher scheme
CoCo-InEKF (arXiv 2605.15122, §III-B) reports degrades learning, structurally: a
128 ms segment starting at **zero** error gives the contact update nothing to
correct, so the loss-minimising ``Σ_C`` is infinite.

Seeding (`state0`): ``R, v, p`` come from ground truth at the segment start. The
contact anchors do **not** — they are ``d_i = R_true·y_i(q̂) + p_true`` with
``y_i`` the FK at the *recorded filter* joint estimate ``inputs.joint.q``, the
same vector the filter will measure against on the first tick, so the first
contact residual is exactly zero. Seeding them from truth ``q`` would inject the
filter-vs-truth joint offset as a step at tick 1 (the failure
`pipeline.main_estimator.init_fused_carry` documents for the ankles). The joint
KF is **not** reseeded: it ran continuously through collection and its outputs
are frozen into `inputs`. ``P0`` is measured, not guessed — see `measure_p0`.
"""
...
def is_rollout(path: Path | str) -> bool:
    ...
    then fails deep inside `load_rollout` — which is what happened the first time
    a checkpoint was written there.  Reading the archive's central directory is a
    few hundred bytes, not the 130 MB payload.
    """
    try:
...
def chunked_fk(kinematics, q: np.ndarray, chunk: int = 2_000) -> np.ndarray:
    ...
    Chunked for the same reason `collect.contact_channels_chunked` is (docstring
    (a)).  Unlike the channel path this one is *exactly* splittable — ``y`` has no
    cross-tick term — so no lead-in is needed and the result is bit-identical to a
...
def build_channel_cache(paths: Sequence[Path | str], collector: collect.Collector, *,
    ...
    **raw**, not normalized: the constants are fit *from* this cache (pass 2), so
    caching a normalized version would be circular.
...
def fit_normalization(paths: Sequence[Path | str], *, cache_dir: Path | str = CACHE_DIR,
    ...
    Pools the **usable** (post-warm-up) region by default, not the whole stream:
    the constants must describe what the network is actually fed, and the warm-up
    prefix includes the standing settle, which is precisely the "calibration set
    never exercised it" failure `NormConstants.floored` exists to expose.  Pass
    ``usable_only=False`` only to compare the two.
...
class PreparedRollout:
    ...
    valid, but not equal) floating-point result.
    """
...
def valid_start_range(T: int, warmup: int, cfg: ContactNetConfig) -> tuple[int, int]:
    ...
    ``t_lo = warmup + (H-1)·stride`` (docstring (c)); ``t_hi = T - L``.  Raises
    rather than returning an empty range: a rollout too short to yield a single
    segment is a collection bug, not a sample to skip quietly.
    """
...
def _constant_contact_chol(cfg: ContactNetConfig, L: int, N_c: int) -> np.ndarray:
    r"""``(L, N_c, 3, 3)`` of ``contact_chol_const · I₃`` — the single place the sim's
    stance/swing ground truth is destroyed (docstring (b)).

    Matches `sim.sensors.SimSensorReader`'s layout (a scalar times the identity)
    so the only thing that changes is that the scalar no longer moves.
    """
...
def make_segment(prep: PreparedRollout, t0: int, cfg: ContactNetConfig,
    ...
    if cfg.freeze_contact_chol:
        # Run-1 behaviour, retained only for reproducibility.  See
        # `ContactNetConfig.freeze_contact_chol`.
...
def sample_starts(rng: np.random.Generator, preps: Sequence[PreparedRollout],
    ...
    Cycled if ``B > n_rollouts``, so no rollout is over-represented by more than
    one segment.
    """
    n = len(preps)
    if n == 0:
...
class ChainedBatcher:
    ...
    distribution at a segment start (docstring (f)).  A chain is seeded from truth
    at a random legal start, warmed in for ``cfg.warm_in_ticks`` untrained, walked
    ``cfg.L`` ticks per step, and re-seeded on episode end, rollout end, or a
    non-finite carry (a diverged chain would poison every later step).
    ...
    independently at different rollouts and phases.  With ``B = 32`` over 12
    rollouts that is ~2.7 chains per rollout at unrelated gait phases.
    """
    ...
    def __init__(self, preps: Sequence[PreparedRollout], cfg: ContactNetConfig,
        ...
        slice and returns the final carry.  Injected rather than built here so this
        module keeps its "no MJX, no estimator build" property; `rollout.make_warm_in`
        supplies it.
        """
        if not preps:
            raise ValueError("no prepared rollouts")
...
def measure_p0(fused, prep: PreparedRollout, cfg: ContactNetConfig, *,
    ...
    with the network.  Measuring beats picking, and it is cheap: run the InEKF
    alone over ``ticks`` of recorded input from a diffuse prior, under exactly
    the conventions a training segment runs under, and take the final ``P``.

    Those conventions changed on 2026-07-29:

    * ``contact_chol`` burns in on the **recorded heuristic**, which is what the
      supervised warm-start starts the network at and what `rollout.make_warm_in`
      warms a chain in on.  Freezing it at the stance constant would seed every
      segment from the covariance of a filter that believes its swing feet are
      world-static.
    ...
    **A `P0` measured before that date is stale and must not be reused** — write a
    new ``artifacts/p0_*.npz`` rather than reaching for ``p0_dr.npz``.  Passing
    ``freeze_contact_chol=True`` still reproduces the old burn-in.

    Returned as one ``(3N+9, 3N+9)`` matrix shared by every segment.  A
    per-gait-phase ``P0`` would be more faithful — it would mean storing ``P`` at
    every tick, 111 MB per rollout — and is the obvious refinement if the seeding
    transient ever shows up in the loss.
    """
