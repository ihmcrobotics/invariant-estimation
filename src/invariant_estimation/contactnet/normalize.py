r"""
contactnet/normalize.py
=======================
Frozen per-channel standardization for ContactNet features
(``network_plan.md`` §5.5).
...
§5.5 rules out both tempting alternatives, for the same reason each time.
*Per-window* standardization strips absolute magnitude, which is precisely what
signals slip — a sliding foot has larger excursions than a planted one.
*Running* statistics add state and create a train/deploy mismatch.  Frozen
constants are the design, not a simplification.

The output is an **artifact**, not code — see `NormConstants`.
"""
...
# Per-channel sensor noise std, keyed by a `features.channel_names()` prefix.
# Measured on the 2026-07-17 Alex001 log (PORT_NOTES.md, "H is not 20").
#
# These are NOISE levels, not signal levels: they exist to stop `fit` dividing a
# channel by its own near-zero calibration variance.  See `fit`.
...
def channel_floor(
    ...
    magnitude (4e-6 rad to 0.2 N.m); one number cannot serve both.
    ...
    ------
    KeyError
        If any channel matches no prefix -- a new channel without a floor is a
        gap, not a default.
    """
    out = []
    for n in names:
...
class NormConstants:
    """
    Frozen per channel standardization.

    Computed once over a calibration set, written to disk, loaded
    at train *and* deploy, and baked into the Java export. Constants
    that cannot be traced back to the run that produced them *cannot be debugged*, 
    so the provenance fields travel *with* the numbers.

    A frozen dataclass rather than a NamedTuple on purpose: this is never
    a pytree. A NamedTuple carrying `names` would make those strings pytree *leaves*,
    and any `jax.tree.map` over it would treat them as arrays, the same reason
    that `ContactNetParams` avoids by *only* holding arrays.
    """
    mean: Array
...
def fit(channels: Array, names: tuple[str, ...], *, floor: Array | None = None, source: str = "") -> NormConstants:
    """
    Compute frozen constants from a calibration set.
    ...
    A non-empty `floored` result is a prompt to look: it means either the channel
    is genuinely at the noise level, or the calibration set never exercised it.
    """
    if channels.ndim != 3:
...
def apply(channels: Array, c: NormConstants) -> Array:
    ...
    exists to prevent: NumPy would happily broadcast a mismatched trailing axis
    in some shape combinations, and the result would train fine and be wrong.

    Cheap enough to be free -- ``F`` subtracts and ``F`` divides per tick -- but
    NOT optional: §7 bakes these constants into the Java export, and any
    disagreement between the training transform and the deployed one is
    undetectable from either side alone.
    """
