...
| `predict.py`      | ⏳ next | `x⁻ = F x`, `P⁻ = F P Fᵀ + Q_d`. Calls `noise.build_F` + `noise.build_process_noise(..., M=robot.mass_matrix(q̂) or None)`. |
| `update.py`       | ⏳ todo | EKF update with **Joseph form** covariance update (§4). |
