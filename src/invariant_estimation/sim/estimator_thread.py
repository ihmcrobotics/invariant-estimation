...
**You probably do not need it.** Measured against the synchronous path: **0.97x headless, 1.03x
with a render per tick** — nothing. The **1.90x** that motivated the design was a micro-benchmark
of JAX overlapped against `mj_step`; the real speed fix was pinning the ONNX session
(`run_policy._ort_session`, PORT_NOTES §"G10 (part)" 2026-07-27). Kept because it is correct,
tested and accuracy-neutral at the default backlog, and a heavier sim would change the arithmetic.

**Opt-in, viewer-only, off by default.** `run_headless` and every test call `rt.advance` directly,
which is what `test_sources_truth_bypasses_the_estimate` pins at `atol=0`. A threaded run is not
bit-reproducible — the estimate the policy reads is a few ticks stale, by design — so
authoritative numbers come from the synchronous headless path.

The four things that are easy to get wrong here:

1. **Never drop a sample.** The filter is a sequential recursion over samples. The queue is a
   plain `deque` with **no `maxlen`** -- a `maxlen` deque discards the *oldest*, the worst
   possible choice here. Past `max_backlog_ticks` the producer BLOCKS until the worker drains it,
   which degrades to sub-real-time -- the honest failure mode.
...
2. **Fixed chunks of `substeps`, always.** `EstimatorRuntime._advance` is a `jax.jit(lax.scan)`
   ...
   violation and an ~11 s stall mid-viewer. The worker leaves any remainder in the queue.
...
3. **Score against PAIRED truth.** Each queued item carries the truth snapshot taken when its
   ...
   published error by the staleness.

4. **A dead worker must be loud, and must not wedge the producer.** The exception is captured and
   re-raised on the sim thread at the next `submit`/`latest`, and the back-pressure wait is gated
   on `_alive()`, not on `_error is None`: a worker that dies WITHOUT recording an error would
   otherwise block the producer forever. Mutation testing found that one, and the test that should
   have caught it hung instead of failing.
...
class ThreadedEstimator:
    ...
    def prime(self, batch, truth) -> Published:
        ...
        The policy needs an estimate on the very first control tick and the sim has already primed
        exactly `substeps` samples by then, so there is no reason to invent a null estimate.
        """
    ...
    def submit(self, sample, truth) -> None:
        ...
        with self._cv:
            ...
            self._cv.notify()
            # BACK-PRESSURE, not dropping. The liveness condition is `_alive()`, NOT
            # `_error is None`: a worker that dies WITHOUT recording an error leaves `_error` None
            # forever and a producer gated on it blocks forever. Mutation testing found this; the
            # test that should have caught it hung instead of failing.
    ...
    def _reraise(self) -> None:
        ...
        # A worker can also stop without recording anything: the estimate would freeze while the
        # robot kept walking on it.
    ...
    def _run(self) -> None:
        try:
            while not self._stop.is_set():
                ...
                est = self.rt.advance(batch)       # OUTSIDE the lock -- the expensive call
                with self._cv:
                    self._consumed += self.substeps
